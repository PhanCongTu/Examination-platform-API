...
public class ClassroomServiceImpl implements ClassroomService {
     ...
     *
     * @return the list of DTO response {@link ClassroomResponseDTO}
     */
    @Override
    public ResponseEntity<?> getAllEnableClassrooms(Integer pageNumber) {
        log.info("Start get all Classroom (non-Admin)");
        Pageable sortedByPriceDescNameAsc =
                PageRequest.of(pageNumber, PAGE_SIZE, Sort.by(PAGE_SORT_BY).descending());
        List<ClassRoom> classRooms = classRoomRepository.findAllEnableClassRooms(sortedByPriceDescNameAsc);
        // Map topic to topic response DTO
        List<ClassroomResponseDTO> response =  classRooms.stream().map((classRoom)-> new ClassroomResponseDTO(
                classRoom.getId(),
        ...
        log.info("End get all Classroom (non-Admin)");
        return ResponseEntity.ok(response);
    }
